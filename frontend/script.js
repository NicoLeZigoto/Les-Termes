...
socket.on('room_joined', (data) => {
    ...
    if (data.votedPlayers && data.votedPlayers.length > 0) {
        setTimeout(() => {
            data.votedPlayers.forEach(voterId => {
                const avatarEl = document.getElementById(`avatar-${voterId}`);
                if (avatarEl) avatarEl.classList.add('validated');
                if (voterId === MY_ID) window._myVoteValidated = true;
            });
        }, 100); 
    }
});
    ...
    activePlayers.forEach((p, index) => {
        ...
        if (pointerEl) {
            ...
            if (p.currentRotation !== undefined) {
                pointerEl.style.transform = `rotate(${p.currentRotation}deg)`;
            }
        }
